					...
					<form method="POST" action="UsuarioController">
						...
						<p>
							<label for="nombre_real"> Nombre real:</label> <input type="text"
								name="nombre_real" value="${param.nombre_real}" />
						</p>
						<p
							class="error-p <c:if test="${empty nombre_real_invalido}">invisible</c:if>">
							...
							<label for="rol"> Rol:</label> <select id="rol" name="rol">
								<c:forEach var="rol" items="${roles}">
									<option value="${rol.name()}"
										${rol.name().equals(param.rol) ? 'selected' : ''}>${rol.getNombreRol()}</option>
								</c:forEach>
							</select>
						</p>
						<p
							class="error-p <c:if test="${empty rol_invalido}">invisible</c:if>">
						...
						<p>
							<sec:one roles="administrador">
								<input class="button" type="submit"
									name="cancelar_nuevo_usuario" value="Cancelar" />
								<input class="button" type="submit" name="guardar_nuevo_usuario"
									value="Guardar" />
							</sec:one>
						</p>
					</form>
